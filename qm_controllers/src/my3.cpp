                                              ...
                                              const SystemObservation &eeState)
{
    ...
    // get yaw
    Eigen::Vector3d eulerAngle = quat.matrix().eulerAngles(0, 1, 2);
    ...
    const vector_t BaseTargetPose = [&]()
    {
        ...
        target(3) = yaw+0.5*M_PI + cmdVel(3) * timeToTarget;
        target(4) = 0;
        target(5) = 0;
        // double dd=0.45;
        // target(0) = -1.93-dd/sqrt(2);
        // target(1) = 0.02+dd/sqrt(2);
        // target(2) = COM_HEIGHT;
        // // target(3) = BaseCurrenPose(3) + cmdVel(3) * timeToTarget;
        // target(3) = -3;
        // target(4) = 0;
                                                ...
                                                const SystemObservation &eeState)
{
    ...
    const vector_t BaseTargetPose = [&]()
    {
        ...
        target(0) = EeTargetPose(0) - dx;
        target(1) = EeTargetPose(1) - dy;
        target(2) = COM_HEIGHT;
        target(3) = yaw;
        target(4) = 0;
        target(5) = 0;
